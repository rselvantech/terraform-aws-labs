...
"terraform plan -refresh-only shows a manually-added tag as drift. What does a normal terraform plan show for the same tag, and how does the framing differ?","Both commands detect the same drift — normal plan proposes REMOVING the untracked tag (reconciling AWS back toward .tf, shown as tag -> null), since it compares config vs state. -refresh-only reports the identical fact as an ADDITION relative to last-known state (+ tag), explicitly labeled as having originated outside Terraform, and proposes no action. Neither command misses the drift — only the direction and framing differ.","demo03,refresh-only,drift,live-verified"
...
"You run terraform fmt -check (no -recursive) on a project with a modules/ subdirectory containing misformatted files. Does this check catch them?","No. Without -recursive, fmt only checks the current directory — any subdirectory (modules/, break-fix/, etc.) is silently skipped. This gives false confidence that the whole project is formatted when it was never actually checked. Always use -recursive for any project with nested .tf files.","demo03,fmt,recursive"
"Name four SNS/SQS-related resources or arguments NOT used in this demo, and what each is for.","aws_sns_topic_policy (controls who can publish/subscribe to the topic itself, distinct from the queue's own policy), redrive_policy argument on aws_sqs_queue (routes failed messages to a dead-letter queue), filter_policy on aws_sns_topic_subscription (delivers only messages matching a filter to a given subscriber), aws_lambda_event_source_mapping (triggers a Lambda function from new SQS messages).","demo03,sns,sqs,related-resources"
"In HCL, what is the rule for distinguishing a block from a map value assignment when both use curly braces?","{ } appearing directly after a bare word with no = means BLOCK — a structural container (e.g. default_tags { } inside a provider block). { } appearing after an = means the value being assigned is a MAP literal (e.g. tags = { Key = \"value\" }, or common_tags = { ... } in a locals block). Blocks never use =; argument assignments always do.","demo03,hcl,syntax,blocks-vs-maps"
```
