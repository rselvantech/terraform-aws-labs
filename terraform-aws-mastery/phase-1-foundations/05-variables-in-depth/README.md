...
> **`-var` on the CLI cannot pass a real `null` to a `string`-typed
> variable.** `terraform plan -var="custom_role_name=null"` does **not**
> pass the HCL `null` value — it passes the four-character *string*
> `"null"`. You can confirm this yourself: output the variable directly
> and you'll see `custom_role_name = "null"` (quoted — a string), not
> `custom_role_name = null` (unquoted — the real null value). This
> means `nullable`'s behavior genuinely can't be exercised via `-var`
> for a string variable at all — to pass a real `null`, you need a
> `.tfvars` file (`custom_role_name = null`, unquoted, real HCL) with
> `-var-file`, or a module call passing `null` directly in HCL. This is
> demonstrated correctly in Break-Fix below, where the same mistaken
> assumption is diagnosed from real, verified output.
...
"A caller runs terraform plan -var='custom_role_name=null' on a string-typed variable. What value does the variable actually receive?","The literal four-character string 'null' — NOT the HCL null value. -var has no special handling of the bare word null for string-typed variables; it takes the CLI input literally. nullable's substitution behavior never gets exercised, since no real null was ever passed. To pass a genuine null, use a .tfvars file (null unquoted) with -var-file, or a module call in real HCL.","demo05,variables,nullable,cli,break-fix"
