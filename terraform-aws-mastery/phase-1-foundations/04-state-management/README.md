...
"After renaming a resource and running state mv, you forget to update a reference to the old label in outputs.tf. What happens on the next plan?","A hard 'Reference to undeclared resource' error — NOT a silent failure or automatic fix. state mv only edits state, never .tf files. EVERY .tf file referencing the old resource label must be updated manually — not just the renamed resource block itself — before the rename is actually complete.","demo04,state-mv,rename,completeness"
"Why does suspending S3 versioning in the Console produce no drift in terraform plan if the config only uses the inline versioning {} argument (no standalone resource)?","versioning {} inside aws_s3_bucket is deprecated since AWS provider v4 specifically for this reason. Terraform only performs drift detection when a configuration value exists to compare against — the deprecated inline block provides none. Without the standalone aws_s3_bucket_versioning resource, Console changes to versioning go completely undetected by plan.","demo04,deprecation,s3-versioning,drift"
"Is terraform refresh still the recommended way to detect drift?","No — deprecated as of Terraform 1.0, may be removed in a future version. Use terraform plan -refresh-only (read-only, previews the update) or terraform apply -refresh-only (writes the update to state after confirmation) instead. Do NOT use terraform refresh in new workflows.","demo04,deprecation,refresh"
"What does the -force flag on terraform state push actually bypass, and why is it needed for a state restore?","It bypasses the serial conflict check. A downloaded backup version has a LOWER serial than what the backend currently holds (the backend advanced since that backup was taken). Without -force, Terraform refuses with 'cannot import state with serial N over newer state with serial M'. -force tells Terraform you intentionally want to push an older version — appropriate only in deliberate recovery scenarios, never in normal operation.","demo04,state-push,force,recovery"
"What import ID format does each of these resource types use — aws_s3_bucket, aws_sns_topic, aws_sqs_queue, aws_iam_policy, aws_security_group?","aws_s3_bucket: bucket name (e.g. my-bucket). aws_sns_topic: topic ARN (e.g. arn:aws:sns:us-east-2:123456789012:my-topic). aws_sqs_queue: queue URL (e.g. https://sqs.us-east-2.amazonaws.com/123456789012/my-queue) — NOT the queue ARN. aws_iam_policy: policy ARN. aws_security_group: security group ID (e.g. sg-0a1b2c3d4e5f67890). The authoritative source for any resource type is the Import section at the bottom of its page in the Terraform AWS provider docs.","demo04,import,import-id-reference"
```
